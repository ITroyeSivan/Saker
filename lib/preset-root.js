 ...
 *
 * The Saker platform ships two security modes (pentest / code-audit) and keeps
 * the harness's generic `standard` preset visible: Saker is mounted as the
 * deployment's only preset root, so filtering `standard` out would leave the
 * operator with no general-purpose mode at all — the workbench becomes unusable
 * for everyday (non-security) sessions.
 *
