...
> 本技能随 code-audit 预设走。
> persona 中的硬规则（底线、验证等级、误报排除、扫描器复核、三级深度、委派方式）不在此重复。

## 目录（按需定位，不必从前往后读）

| 章节 | 讲什么 | 什么时候读 |
|---|---|---|
| 审计形态判定与开工问询 | 静态/动态判定，第一动作 | 开工 |
| 阶段默认通道 | 各阶段默认工具与降级链 | 选手段时 |
| 审计前置识别（Triage） | 语言/框架/入口面初判 | 拿到代码后 |
| 静态审计标准 | 静态线的判定标准 | 静态审计 |
| 危险 sink 优先策略 | 从 sink 反推可达性 | 定优先面 |
| RCE 主线聚焦（主观念） | RCE 完整链视角 | 追 RCE |
| 审计对象范围 | 范围界定与排除 | 划范围 |
| 移动端/小程序反编译审计 | 反编译产物审计路径 | 目标是客户端 |
| 场景化审计卡 | LLM Agent/供应链/配置/反编译四卡 | 对号入座 |
| 规则降级 | 规则缺失时的等价手段 | 规则不够用 |
| 动态验证与证据留痕 | 跑起来验 + 证据要求 | 需动态确认 |
| 确证闭环流程 | 从发现到确证 | 每个 finding |
| Diff 审计模式 | 增量代码审计 | 审补丁 |
| 交叉复核规范 | 独立复核要求 | 复核发现 |
| 调用链引用规范 | 链路引用写法 | 写报告 |
| 成果页登记（会话隔离） | finding 登记与回写 | finding 进报告前 |
| 报告模板 | 报告结构与字段 | 出报告 |
| 工具手册 | semgrep 等工具用法 | 调工具前 |
| 子代理编排 | 分工与并行 | 可并行时 |
