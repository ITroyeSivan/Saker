...
安全预设（pentest / code-audit / ctf-solver）的**逐轮治理信封**——「红队完整上线」逐轮注入逻辑的 DSH
...
- **正文级 skill-boost 不做（设计取舍）**：对方每轮灌命中技能前 2000 字，我们只给指针——
  各预设 refs 规模大（数百篇），逐轮灌正文会撑爆上下文；模型按指针自取 + persona 纪律。
  翻转开关留 v0.3（includeRefs 已参数化）。
- **会话内模式切换**：DSH 平台约束（仅 blank 会话可选预设），生态规则（子代理+跨模式
  playbook）补偿，本插件不解决。
- **OPSEC 关键词调级**：不补——预设速率纪律在 persona/playbook 有静态默认，
  运行时调级价值低。
