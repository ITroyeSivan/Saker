...
Windows 命令通道：基础命令马按 4000B 原始数据分块，避免 cmd.exe 8191 字符命令行上限；certutil 读取优先截取证书块，中文状态行数字不会污染 base64；中文 Windows 24 小时 `dir` 输出可直接解析。

模型连接参数兼容 `pass_param` / `cmd_param` / `secret_key`（snake_case）与内部 camelCase，不会再静默回退到默认参数名。
