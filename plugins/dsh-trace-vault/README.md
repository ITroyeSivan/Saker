...
- **中断恢复**：`tool/call` 开始即落 `running` 行，结果回来后更新为 ok/blocked/error；
  心跳过期或进程崩溃后自动转 `interrupted`。多进程并发首开不会把尚未写完的 SQLite
  头误判成坏库。
- **跨会话评估指标**：从真实 trace 行计算首次有效动作时间（会话最早一行到第一次
  `outcome=ok` 工具结果）、工具失败率（blocked/error/interrupted）和错误率；
  没有样本时写 `null`，不拿 0 冒充测量结果。
...
`node test/run.mjs`——分类器/截断/配对落库/检索转义/统计/跨会话评估/清理/上限/装配/信封。
