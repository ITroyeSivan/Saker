...
- 工具 `semgrep_scan`：本地 semgrep + 预设离线三层规则集（自动定位 refs/：java 402 条自建 / php / oss 1096 条；layer=custom 指自定路径）
...
- 若 `operation_intent` 登记了同 owner 的 queued 长任务，扫描结束自动收口为 `succeeded/failed`
- 只读：静态扫描不写目标仓；产物只落任务工作区
- 异步执行：semgrep 子进程不再用 `spawnSync` 冻结宿主事件循环；超时和输出上限仍生效
- Windows UTF-8：子进程强制 `PYTHONUTF8=1` / `PYTHONIOENCODING=utf-8`，避免 zh-CN 系统按 GBK 读取含中文的内置规则而崩溃
