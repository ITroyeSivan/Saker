...
function createController(api, currentSession, onApplied) {
	...
	function load() {
		return api.agentPresets.list().then(function (res) {
			if (!res.ok) { set({ error: res.error && res.error.message }); return; }
			var presets = res.value.presets || [];
	...
	function applyStaged() {
		...
		return api.agentPresets.select(s.id, staged).then(function (res) {
			staged = undefined;
			if (!res.ok) { set({ busy: false, error: res.error && res.error.message, current: fallback }); return; }
			set({ busy: false, current: res.value });
			onApplied && onApplied(s.id, res.value);
...
module.exports = { name: "dsh-mode-group-client", inject: ["slots", "locale", "connection", "remote", "remote.agentPresets"], apply: apply };
